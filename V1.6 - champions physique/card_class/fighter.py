...
class Fighter(Unit):
    ...
    def activate_inv(self, ally_player, ennemy_player):
        if self.name == "Matelot":
            ...
            for unit in ally_player.frontrow:
                if unit.name == "Tentacule":
                    unit.atk = 4
                    unit.pv = 4
        elif self.name == "Pêcheur légendaire":
            ally_player.draw_card(1, Fighter, "abyssal", 1)
            ...
            ennemy_player.infection_2 = True
            ...
            for _ in range(4):    
                if len(ennemy_player.frontrow) > 0:
                    target = random.choice(ennemy_player.frontrow)
                    target.take_damage(1)
        elif self.name == "Chaman Cognefer":
            ally_player.draw_card(1, Ritual)
        elif self.name == "Guerrier Cognefer":
            self.cost += 1
        elif self.name == "Atra Hasis":
            ally_player.deck.append(ally_player.create_card("Fragment du destin 1"))
            ally_player.deck.append(ally_player.create_card("Fragment du destin 2"))
            ally_player.deck.append(ally_player.create_card("Fragment du destin 3"))
            ally_player.deck.append(ally_player.create_card("Fragment du destin 4"))
            ally_player.deck.append(ally_player.create_card("Fragment du destin 5"))
            random.shuffle(ally_player.deck)
        elif self.name == "Kodo de livraison":
            ally_player.draw_card(1, Fighter, "bête", 1)
        elif self.name == "Kodo du désert":
            pass
    ...
    def activate_mort(self, ally_player, ennemy_player):
        ...
        elif self.name == "Garuda":
            ally_player.summon_unit("Vautour", "back")
            ally_player.summon_unit("Vautour", "back")
