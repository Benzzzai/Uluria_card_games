...
class App:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        ...
                        if self.selected_card:
                            self.game.move_void(self.selected_card)
                            self.selected_card = None
                elif event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN or event.key == pygame.K_SPACE:
                        self.selected_card = None
    ...
    def check_end(self):
        if self.game.player_1.health == 0 or self.game.player_2.health == 0:
            self.run = False
            self.end_menu = True
    ...
    def display_cards(self, row):
        for card in row:
            ...
            if isinstance(card, Fighter):
                color_rect = C_GREY_CO
            elif isinstance(card, Structure):
                color_rect = C_GREY_ST
            elif isinstance(card, Spell):
                color_rect = C_GREY_SP
            elif isinstance(card, Divine):
                color_rect = C_GREY_ALT
            else:
                color_rect = C_GREY_LIGHT
            pygame.draw.rect(self.screen, color_rect, card.rectangle)

            # NAME
            color_name = C_WHITE
            if isinstance(card, Champion):
                color_name = C_YELLOW_ATK
            ...
            if isinstance(card, Unit):
                ...
                # EFFECTS
                if card.protecteur:
                    effect_box_rect = pygame.Rect(card.rectangle.right - 20, card.rectangle.top + 5, 15, 15)
                    pygame.draw.rect(self.screen, C_GREY_LIGHT, effect_box_rect)
                elif card.insaisisable:
                    effect_box_rect = pygame.Rect(card.rectangle.right - 20, card.rectangle.top + 5, 15, 15)
                    pygame.draw.rect(self.screen, C_YELLOW_ATK, effect_box_rect)
    ...
    def zoom_card(self, card, player):
        ...
        if isinstance(card, Fighter):
            ...
            color_rect = C_GREY_LIGHT
        pygame.draw.rect(self.screen, color_rect, big_rect)
    ...
    def display_app(self):
        ...
        d2_rect = pygame.Rect(MANA_X, POS_DECK_Y[0], DECK_SIZE, DECK_SIZE)
        pygame.draw.rect(self.screen, C_GREEN_BOOST, d2_rect)
        d2_surface = FONT_STAT.render(str(self.game.player_2.avatar.level), True, C_BLACK)
        ...
        pygame.draw.rect(self.screen, C_BLACK, d2_rect)
        d2_surface = FONT_STAT.render(str(len(self.game.player_2.discard)), True, C_WHITE)
        d2_rect = d2_surface.get_rect(center=d2_rect.center)
        self.screen.blit(d2_surface, d2_rect)
        ...
        d1_rect = pygame.Rect(MANA_X, POS_DECK_Y[1], DECK_SIZE, DECK_SIZE)
        pygame.draw.rect(self.screen, C_GREEN_BOOST, d1_rect)
        d1_surface = FONT_STAT.render(str(self.game.player_1.avatar.level), True, C_BLACK)
        ...
        pygame.draw.rect(self.screen, C_BLACK, d1_rect)
        d1_surface = FONT_STAT.render(str(len(self.game.player_1.discard)), True, C_WHITE)
        d1_rect = d1_surface.get_rect(center=d1_rect.center)
        self.screen.blit(d1_surface, d1_rect)
